...
export class ChroniclesController {
  ...
  // --- NUEVA RUTA: Obtener artículos pendientes de aprobación (Solo Admins) ---
  @Roles(Role.SUPERADMIN, Role.ADMIN)
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Get('pending')
  findPending() {
    return this.chroniclesService.findPending();
  }

  @UseGuards(JwtAuthGuard)
  @Get()
  findAll() {
    // Nota: En tu servicio, asegúrate de que esto filtre por { isApproved: true } 
    // para que los usuarios normales solo vean las publicadas.
    return this.chroniclesService.findAll();
  }
  ...
  // --- NUEVA RUTA: Autorizar / Aprobar un artículo (Solo Admins) ---
  @Roles(Role.SUPERADMIN, Role.ADMIN)
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Patch(':id/approve')
  approve(@Param('id') id: string) {
    return this.chroniclesService.approve(+id);
  }
